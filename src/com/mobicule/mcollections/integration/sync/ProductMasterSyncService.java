...
public class ProductMasterSyncService implements IProductMasterSyncService 
{
	...
	public Message<String> execute(Message<String> message) throws Throwable {
		log.info(" -------- In ProductMasterSyncService / execute -------- ");
		...
		try
		{
			...
			if (entity.equals("LEAD_PRODUCT_MASTER"))
			{
				log.info("----ProductMaster sync ---" + messagePayload);

				mapOutput = (Map<String, String>) applicationConfiguration.getValue("LEAD_PRODUCT_MASTER");
		...
		{
			log.info(" -------- Error in ProductMasterSyncService / execute -------- ");

 ...
 *
 */
public class GCMDeviceService implements IGCMDeviceService
{
	...
	public Message<String> execute(Message<String> message) throws Throwable
	{
		...
		log.info("------inside GCM device Token updation service------");
		...
		try
		{
			...
			if (type.toString().equalsIgnoreCase("gcmToken"))
			{
				log.info("---token---" + data.get("gcmToken"));

				systemUserNew.setGcmToken((String) data.get("gcmToken"));

				flag = systemUserService.updateDeviceToken(systemUserNew);
				...
				if (flag)
				{
					...
					return responseBuilder(message, status, "Device Token got submitted successfully");
				}
				else
				{

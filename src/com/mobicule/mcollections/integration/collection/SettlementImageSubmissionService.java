 ...
 *
 */
public class SettlementImageSubmissionService implements ISettlementImageSubmissionService
{
	...
	public Message<String> execute(Message<String> message) throws Throwable
	{
		...
		try
		{
			...
			log.info("--- Settlement Images ----" + settlement.getImages());
			...
			if (settlementService.checkDuplicateSettlementImageJSON(settlement))
			{

				submissionFlag = settlementService.submitSettlementImage(settlement);
				...
				if (submissionFlag)
				{
					...
					return responseBuilder(message, status, "Settlement image got submitted successfully", settlement
			...
			else
			{
				...
				return responseBuilder(message, status, returnMessage, settlement.getImages().get(0).getImageId());
			}
			...
			throws JSONException
	{
		...
		for (int index = 0; index < imageDetails.length(); index++)
		{
			...
			imagePath = (extractImagePath(settlement, imageId, imageByteArray, (String.valueOf(index))));
			...
			{
				imagePath = settlement.getRequId() + "/" + imageName + Constants.EXTENSION_IMAGE;
	...
	private String extractImagePath(Settlement settlement, String imageId, String type, String index)
	{
		try
		{

			String fileName = imageId + Constants.SYMBOL_UNDERSCORE + settlement.getRequId()
			...
			if (index.equals(Constants.EMPTY_STRING))
			{
				...
				filePath = generateFilePath(
						(String) applicationConfiguration.getValue(Constants.Settlement.SETTLEMENT_IMAGE_PATH),
						fileName, settlement.getRequId());
			}
			else
			{
				...
				filePath = generateFilePath(
						(String) applicationConfiguration.getValue(Constants.Settlement.SETTLEMENT_IMAGE_PATH),
						(fileName + "_" + index), settlement.getRequId());
			}
